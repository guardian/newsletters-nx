...
>>>>>>> cfd2890 (add express rate-limit middleware to two newsletters.ts endpoints to address github codeQL warnings as a result of running new prettier config across the codebase (apparently codeQL only reports on lines that appear in a pr diff))
			return res.send(makeSuccessResponse(storageResponse.data));
		},
	);
}
