...
describe('draftNewsletterToRow', () => {
	...
	it('leaves the pillar/category label unset when the draft has neither', () => {
		const row = draftNewsletterToRow(
			draft({ theme: undefined, category: undefined }),
		);

		expect(row.pillarCategoryLabel).toBeUndefined();
	});
