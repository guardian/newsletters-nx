...
import {
	addNewGroup,
	deleteGroup,
	deleteNewsletterFromGroup,
	insertNewsletterIntoGroup,
	moveNewsletterTo,
	updateLayoutGroup,
} from '../../lib/modify-layout';
...
export function layoutReducer(
	...
	const addToHistory = (layout: Layout): LayoutState['history'] =>
		[layout, ...state.history].slice(
			0,
			MAX_HISTORY_LENGTH + 1,
		) as LayoutState['history'];
	...
	switch (action.type) {
		...
		case 'add-group':
			return {
				...
				history: addToHistory(
					addNewGroup(current, action.index ?? current.groups.length),
				),
				redoStack: [],
			};
		case 'update-group':
			return {
				...state,
				history: addToHistory(
					updateLayoutGroup(current, action.groupIndex, action.mod),
				),
				redoStack: [],
			};
		...
		case 'insert-newsletter':
			return !state.selectedNewsletter
				? state
				: {
						...state,
						history: addToHistory(
