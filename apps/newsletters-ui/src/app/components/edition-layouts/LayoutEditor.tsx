...
export const LayoutEditor = ({
	...
	const { feedback, updateInProgress, history, selectedNewsletter, redoStack } =
		state;
	...
	return (
		<Box>
			<Box
				sx={{ display: 'flex', gap: 2, minHeight: 80, alignItems: 'center' }}
			>
				<Button
					variant="contained"
				...
				{feedback === 'failure' && (
					<Alert severity="error">
						...
						<Typography>
							If the problem persists, please contact Central Production
						</Typography>
					</Alert>
				)}
				<Box sx={{ marginLeft: 'auto', display: 'flex', gap: 1 }}>
					<Badge
						badgeContent={hasHistory ? history.length - 1 : undefined}
						color="secondary"
					>
						<Button
							variant="outlined"
					...
					<Badge
						badgeContent={canRedo ? redoStack.length : undefined}
						color="secondary"
					>
						<Button
							variant="outlined"
			...
			<Box sx={{ display: 'flex', gap: 1, paddingTop: 2 }}>
				<NewsletterPicker
					...
					setSelectedNewsletter={(selectedNewsletter) =>
						dispatch({ type: 'select-newsletter', selectedNewsletter })
					}
					stackProps={{ sx: { flex: 1 } }}
				/>
				<Stack sx={{ flex: 3 }}>
						...
						<Fragment key={groupIndex}>
							<Divider>
								<Button
									variant="contained"
									onClick={() =>
										dispatch({ type: 'add-group', index: groupIndex })
									}
								>
									add group
								</Button>
