...
Feature: Edit an existing draft newsletter in the redesigned wizard
	...
	Scenario: The 'Name & frequency' step is only shown once in the navigation
		Then the 'Name & frequency' step is only shown once in the navigation

	Scenario: The name field is pre-filled with the existing draft's name
		Then the name field is pre-filled with the existing draft's name
