...
seen in local `.env` examples are for local runtime behavior, not a deployed CDK
stage in this repo.

The two apps are protected differently. The tool sits behind Google auth. The read-only API has no user auth at all — instead two listener rules allow requests carrying a matching `X-Gu-API-Key` header and return a fixed 403 to everything else:

Instance configuration is injected as environment variables through user data — `NEWSLETTER_BUCKET_NAME`, `STAGE`, `STACK`, `APP` and `ENABLE_EMAIL_SERVICE`. The tool's value is read from the per-stage `enableEmailService` SSM parameter; the read-only API is always passed `'false'`.
